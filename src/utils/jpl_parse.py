...
class JPLParse:
    ...
    def __init__(
        ...
        if data_args is None:
            ...
            assert isinstance(
                data_args, JPLData
            ), "arg must be instance of JPLData class"
            self.data_args = data_args

            self.date_columns = pd.date_range(
                self.data_args.start, self.data_args.end, freq="d"
            )

            assert (
                self.data_args.data_path is not None
            ), "must call JPLData._get_data first"
    ...
    def _parse(self):
        try:
            ...
            print(
                "data path invalid, try copying the path provided when calling JPLData._get_data()"
            )
        ...
        for parsing in split_data:
            if "$$SOE" in parsing:
                lines = parsing.split("\\n")
                self.data = [
                    line.split(",")[2:-1] for line in lines if len(line.split(",")) > 1
                ]

        if len(self.data[0]) != len(c.columns()):
            raise Exception(
                "something went wrong when parsing the data, check to see if data exists"
            )
