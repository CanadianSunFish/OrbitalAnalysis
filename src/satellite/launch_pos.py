...
class LaunchPosition:
    ...
    def _get_target_orbit(self):
        ...
        mask_country = np.column_stack(
            [df["country"].str.contains(country, na=False) for col in df]
        )
        mask_city = np.column_stack(
            [df["city_ascii"].str.contains(city, na=False) for col in df]
        )
        ...
        try:
            ...
            try:
                ...
                if self.output == True:
                    print(
                        f"Could not locate {city}, instead using: {new_city.city}, {country}"
                    )
